...
=> (assert(trastorno "no presenta ningun trastorno"))
...
=> (assert(consejo "TDAH tipo falta atencion"))
)
...
(defrule regla_8
    ...
    (tareas realizadas_con_probolema)
=> (assert(trastorno "presenta TDAH por falta de atencion"))
)
...
(defrule regla_13
    ...
    (tareas realizadas_sin_probolema)
=> (assert(consejo "TDAH tipo combinado"))
)
...
=> (assert(trastorno "presenta un posible trastorno de TDAH combinado"))
)
...
(defrule regla_18   
    (comprobar permanece_sentado)
    (sentado no)
=> (assert(trastorno "presenta un posible trastorno de TDAH hiperactivo"))
    (assert(derivar PT))
)
...
(defrule regla_21
    ...
    (tareas realizadas_con_probolema)
=> (assert(trastorno "presenta comportamientos propios del TDAH hiperactivo"))
)
...
=> 
    (assert(trastorno "presenta comportamientos asociados a el trastorno borderline"))
)
...
(defrule regla_31 
    ...
    (emociones irregulares)
=>
    (assert(trastorno "presenta comportamientos asociados a un retraso madurativo"))
)
...
(defrule regla_32 
    ...
    (emociones no_irregulares)
=>
    (assert(consejo "posible derivacion a AL"))
)
...
=>
    (assert(consejo "vigilar al ninio, posible caso de TEA"))
)
...
(defrule regla_56
    ...
    (emociones no_irregulares)
=>
    (assert(trastorno "no presenta TEA"))
)
...
(defrule regla_66
    ...
    (flexibilidad_cognitiva alterada)
=>
    (assert(trastorno "presenta comportamientos asociados al trastorno TEA"))
)
